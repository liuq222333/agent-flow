...
同一工作流 version 递增
发布后修改草稿不影响旧版本
Runtime 只读取 workflow_versions.graph_json
```
...
NextNodeResolver 使用 target 选择下一节点
```
...
```text
Redis 对外端口为 6380，容器内仍使用 redis://redis:6379/0
API reload 只监听 app 目录
generated_workflows 是运行时生成代码目录，不触发 API reload
```
...
Runtime Core 可以在 workflow_versions 表完成后开始
...
4. Publish
5. Runtime Core
6. Start / Input / LLM / Output / End
7. Run API / Trace API
8. 前端列表页和编辑器
9. 编辑器运行调试
10. 知识库文档处理
11. Knowledge Base Node
12. Intent Node
13. Branch Node
14. Tool / Secret / API Node
15. Message Node
16. 错误处理、重试、权限、部署
```
...
```text
前端保存的 graph_json 与 Runtime 期望结构不一致
```
...
```text
工作流创建、草稿保存、发布和运行
Generated workflow code 发布与运行
同步运行和异步运行
Knowledge Base vector retrieve
Intent + Branch 分支流转
API + Message 工作流
workflow_run / node_run / Trace 查询
Trace code metadata
Trace 节点 input / output 展开
Docker Compose 本地启动、状态查看和停止命令
本地 check 与 smoke e2e 验收命令
```

后续增强不阻塞 MVP 核心闭环，但进入生产前需要补齐：

```text
更完整权限系统
生产级监控、告警和日志聚合
生产级 secret / KMS 与密钥轮换
更多文件类型解析
生产级备份恢复、容量规划和发布回滚演练
```
