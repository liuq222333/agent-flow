...
代码产物本地优先：发布生成 workflow.py，运行以本地生成代码为准
```
...
发布成功后生成 backend/generated_workflows/workflow_xxxxxx/vxxxxxx/workflow.py
生成代码是运行时真相源，可被本地手动编辑
发布版本不可变
```
...
```text
创建 workflow_run
加载 workflow_version 对应的本地 generated workflow code
import workflow.py 并调用 run(input_data, context)
执行节点
维护 State
记录 Trace
处理错误、重试、超时
...
```text
Runtime 默认使用 workflow_versions.code_path 指向的 workflow.py
运行前重新计算本地 code_hash
code_hash 与发布记录不一致时记录 code_modified=true，但不阻止运行
workflow.py 缺失、import 失败或缺少 run 入口时，运行失败并写入明确错误码
workflow.py 不直接写数据库，必须通过 RuntimeContext 调用节点、State 和 Trace 能力
```
...
```text
根据 workflow_versions.graph_json 生成 workflow.py
生成 manifest.json
创建 workflow_id + version 隔离目录
计算 code_hash
...
WorkflowCodegenService
WorkflowCodeWriter
WorkflowManifestBuilder
GeneratedCodeHasher
GeneratedWorkflowPathResolver
```
...
app/
  ...
  services/
    ...
    workflow/
      ...
      workflow_codegen_service.py
      workflow_code_writer.py
      workflow_manifest_builder.py
      graph_validator.py
    runtime/
      workflow_run_service.py
      generated_workflow_loader.py
...
计算下一个 version
创建 workflow_version
调用 WorkflowCodegenService 生成本地代码
写入 code_path / code_hash / code_generated_at
更新 current_version_id
写 audit_logs
```

实现时应保证发布原子性：代码先生成到临时目录，校验和 hash 成功后再移动到最终版本目录；如果代码生成失败，应回滚本次 workflow_version、current_version_id 和审计写入，不留下半发布版本。

---

## 8.3 WorkflowCodegenService

职责：

```text
根据 workflow_versions.graph_json 生成 workflow.py
为每个 workflow_id + version 创建独立目录
生成 manifest.json
计算并返回 code_hash
不覆盖既有版本目录
```
...
选择 version_id
创建 workflow_run
加载 workflow_versions.code_path 对应的 workflow.py
触发同步或异步 Generated Workflow Runtime
查询 run 和 trace
取消 run
```
...
## 8.5 WorkflowExecutor

职责：

```text
执行 generated workflow code
调度节点执行
维护 State
提供受控 RuntimeContext API
处理异常
调用 TraceRecorder
```
...
更新 node_run 成功或失败
写入 metadata_json
写入 code_path_at_run / code_hash_at_run / code_modified
调用 Redactor 脱敏
```
...
```python
class NodeExecutor:
    async def execute(self, node, node_input, resolved_config, state, context) -> dict:
        ...
```
...
Runtime 根据 output_mapping 写 State；Output Node 由 Runtime 将 node_output.outputs merge 到 state.outputs
...
```text
MVP 仅允许 1536 维 embedding，必须和 knowledge_chunks.embedding vector(1536) 一致
不同 embedding model 对应不同维度或索引时，后续需要分表或多列迁移策略
```
