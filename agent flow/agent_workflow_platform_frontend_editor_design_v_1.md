...
出边用于可视化展示，并必须与 branches[].target 保持一致
后端强校验 target 必须存在、每个 target 必须有对应出边、Branch 所有出边必须能映射回 branches[].target
```
...
Output Node 不使用 output_mapping。保存时只提交 `config.outputs`，Runtime 解析后由 OutputNodeExecutor 返回 `node_output.outputs`，再 merge 到 `state.outputs`。
