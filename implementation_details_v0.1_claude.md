...
```text
GET /runs/{run_id}/trace?since_node_run_id={id}

响应：
  仅返回 id > since_node_run_id 的 node_runs
...
async def get_trace(run_id: int, since_node_run_id: int = 0):
    run = await run_repo.get(run_id)
    node_runs = await node_run_repo.list_after(
        run_id=run_id,
        since_id=since_node_run_id,
    )
    return {
        "run": run,
        "node_runs": node_runs,
        "next_since": node_runs[-1].id if node_runs else since_node_run_id,
    }
```
...
while (run.status === "running" || run.status === "pending") {
    const resp = await fetch(`/runs/${runId}/trace?since_node_run_id=${since}`);
...
data: {"node_id": "llm_1", "node_run_id": 3001}
...
ALTER TABLE knowledge_bases
  ADD CONSTRAINT chk_kb_embedding_dim
    CHECK (embedding_dim IN (768, 1024, 1536));
