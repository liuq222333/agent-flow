...
```text
npm run check:local    本地静态检查、单元/集成检查和环境检查入口
npm run compose:up     启动 PostgreSQL、Redis、API、Frontend、workflow worker、document worker
...
手动修改 generated workflow 后，按运行时加载逻辑重新运行工作流或重启服务验证
```
...
migration 已执行
pgvector 扩展已启用
knowledge_chunks.embedding 维度与 embedding model 一致
必要索引已创建
```
